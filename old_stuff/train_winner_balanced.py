...
def visualize_augmentation(images, transform, num_samples=5):
    """Visualize original and augmented images."""
    fig, axes = plt.subplots(2, num_samples, figsize=(20, 8))
...
# Visualize some samples before augmentation
visualize_augmentation(images, augment, num_samples=5)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=50, patience=10):
    ...
    torch.save(model.state_dict(), os.path.join(run_dir, 'final_model.pth'))  # Save final model
